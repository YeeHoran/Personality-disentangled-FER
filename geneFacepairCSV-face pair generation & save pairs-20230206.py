...
def copyFramesFolder(framesFolderPath, verifyNum):
    if not (framesFolderPath in listFramesFolderNames):  #if the identity doe not exist in the list, then 1.insert its name into the list.
        ...
        #print("repeat")        #when the identity exist, then will not add it to the resulted hierachy datasets.  
...
#dsDirLevel3='train-mini version for code test'   #this is a mini-dataset for code test
dsDirLevel3='train1_1'   #this is the source dataset for processing
...
for dirLevel2 in dirsLevel2:       #change to level2 directory, such as './train/train1', './train/train2', etc.
    ...
    print(dirLevel2)   #print 2-order dir, for observation on console.
