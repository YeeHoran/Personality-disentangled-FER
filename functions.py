...
def detect_face(img, detector_backend = 'opencv', grayscale = False, enforce_detection = True, align = True):
	...
	if (isinstance(detected_face, np.ndarray)):
		...
		if detected_face == None:
			#if enforce_detection != True:    #deleted by YH.
			return img, img_region            #added by YH.
			#else:                            #deleted this row by YH.
				#raise ValueError("Face could not be detected. Please confirm that the picture is a face photo or consider to set enforce_detection param to False.")   #deleted this row by YH.
